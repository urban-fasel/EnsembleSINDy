...
% % following discussion paper Messenger 2020
% u(:,:,1)=tanh(sqrt(X.^2+Y.^2)).*cos(m*angle(X+i*Y)-(pi*sqrt(X.^2+Y.^2)));
% v(:,:,1)=tanh(sqrt(X.^2+Y.^2)).*sin(m*angle(X+i*Y)-(pi*sqrt(X.^2+Y.^2)));
...
save('datasets/rxn_diff.mat','t','x','y','u','v')
% save('reaction_diffusion_bigMessenger.mat','t','x','y','u','v')
