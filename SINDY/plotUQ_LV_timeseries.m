function plotUQ_LV_timeseries(Xi,Xistd,x0,tspan,polys,nUQ,pct,nE,tobs,xobs,options,lhpop)


rng(1,'twister')

iEE = 1;
che = 10; % check if sim diverges, then don't add it to estimate UQ
...
plot(tTrue,xTrue(:,1)*n1,'x','Color',cHare,'LineWidth',lw,'MarkerSize',ms); hold on
...
plot(tTrue,xTrue(:,2)*n2,'x','Color',cLynx,'LineWidth',lw,'MarkerSize',ms); hold on
